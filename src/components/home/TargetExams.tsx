        ...
        {exams.map((c, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              background: urgent ? "linear-gradient(135deg, #FEF2F2, #FEE2E2)" : "var(--bg-card)",
              ...
              <div style={{ fontSize: 10, color: "var(--text-faint)", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, marginTop: 2 }}>
                {days === 0 ? "Today!" : days === 1 ? "day left" : "days left"}
              </div>
              <div style={{ fontSize: 12, color: "var(--text-dark)", fontWeight: 700, marginTop: 7, lineHeight: 1.3 }}>{c.name}</div>
              <div style={{ fontSize: 11, color: "var(--text-faint)", marginTop: 2 }}>{formatShortDate(c.date)}</div>
